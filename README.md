...
**cim-compare** has two possible command-line usages to produce comparison reports. Option #1 requires that a comparison XML log file be generated from within Enterprise Architect (see: [EA_COMPARISON_PROCEDURES.md](https://github.com/CIMug-org/cim-compare/blob/gh-pages/EA_COMPARE_PROCEDURES.md)) and then used as input from the command line. Option #2, accepts baseline and target XMI model files and internally generates the XML model comparison log and used it as internal input into the generation of the HTML comparison report output file.  Consequently, Option #2 is typically the preferred approach.
